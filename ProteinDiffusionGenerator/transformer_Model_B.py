...
class ProteinDesigner_B(nn.Module):
    def __init__(self, unet, timesteps=10 , dim=32,pred_dim=25,loss_type=0, 
        ...
        self.max_text_len=max_text_len
        # +++++++++++++++++
        self.ynormfac = ynormfac
        self.Xnormfac = Xnormfac
    ...
    def sample (self, x=None, stop_at_unet_number=1 ,cond_scale=7.5, 
        ...
        if x_data != None:
            ...
            x_data= sequence.pad_sequences(x_data,  maxlen=self.max_length, padding='post', truncating='post')
            
            x_data= torch.from_numpy(x_data).float().to(device)
            x_data = x_data/self.Xnormfac
        ...
        if init_images != None:
            ...
            init_images= sequence.pad_sequences(init_images,  maxlen=self.max_length, padding='post', truncating='post')
            init_images= torch.from_numpy(init_images).float().to(device)/self.ynormfac
        ...
        if inpaint_images != None:
            ...
            inpaint_images= sequence.pad_sequences(inpaint_images,  maxlen=self.max_length, padding='post', truncating='post')
            inpaint_images= torch.from_numpy(inpaint_images).float().to(device)/self.ynormfac
...
def sample_sequence_normalized_Bfac (
    seccs=[0.3, 0.3, 0.1, 0., 0., 0., 0., 0. ],
    model=None,
    ):
